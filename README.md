...
3. Создайте файл `.env` в корне проекта и добавьте ваши API ключи:
   ...
   - Откройте `.env` и замените `your_*_api_key_here` на ваши реальные API ключи
...
**Примечание:** При установке через инсталлятор файл `.env.example` будет автоматически скопирован в `.env` при первом запуске программы (если `.env` не существует).
...
Исполняемый файл будет создан в папке `dist\ChatList-v{version}.exe`

## Создание инсталлятора

Для создания инсталлятора с помощью Inno Setup:

1. Установите [Inno Setup](https://jrsoftware.org/isdl.php) и добавьте его в PATH
2. Выполните сборку приложения (см. выше)
3. Запустите скрипт создания инсталлятора:
...
Инсталлятор будет создан в папке `installer\ChatList-Setup-{version}.exe`

### Возможности инсталлятора:

- Автоматическая установка в Program Files
- Создание ярлыков в меню "Пуск" и на рабочем столе
- Полное удаление программы через "Удаление программ" Windows
- Автоматическая очистка пользовательских данных при удалении:
  - База данных (chatlist.db)
  - Логи приложения
  - Настройки из AppData
- Проверка, что программа не запущена перед удалением
